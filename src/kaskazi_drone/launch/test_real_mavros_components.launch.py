#!/usr/bin/env python3
"""
Test Real MAVROS Components Launch File

This launch file tests the real components without full ArduPilot SITL:
1. Mock MAVROS (simulates MAVROS service endpoints)
2. Real Drone Control Node (connects to MAVROS, not mock)
3. Motion Coordinator (A* planning)
4. Movement Action Client

This helps test the MAVROS integration without requiring full ArduPilot setup.

Usage:
ros2 launch kaskazi_drone test_real_mavros_components.launch.py x_param:=2.0 y_param:=3.0 z_param:=4.0
"""
...
def generate_launch_description():
    """Generate launch description for testing real MAVROS components."""
    ...
    # 1. Mock MAVROS Service (provides MAVROS service endpoints)
    ...
    # 3. REAL Drone Control Node (connects to MAVROS service)
    drone_control_node = TimerAction(
        period=2.0,  # Start after motion coordinator
        actions=[
            Node(
                package='kaskazi_drone',
                executable='drone_control_node',  # REAL drone control node
    ...
    return LaunchDescription([
        ...
        y_arg, 
        z_arg,
        mock_mavros_node,        # Mock MAVROS (provides endpoints)
