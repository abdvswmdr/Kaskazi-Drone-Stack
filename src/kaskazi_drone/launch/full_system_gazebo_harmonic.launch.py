#!/usr/bin/env python3
"""
Complete Drone Navigation System with Gazebo Harmonic Simulation

This launch file integrates:
1. Gazebo Harmonic simulation with Crazyflie drone
2. A* Motion planning system (all C++ components)
3. Mock MAVROS interface for testing
4. Movement action client for sending goals

Architecture:
Gazebo Harmonic ← Mock MAVROS ← Drone Control ← Motion Coordinator ← Action Client
    (Crazyflie)        ↑              ↑                    ↑
                  Waypoints      A* Planning           User Goals

Usage:
ros2 launch kaskazi_drone full_system_gazebo_harmonic.launch.py x_param:=2.0 y_param:=3.0 z_param:=4.0
"""
...
def generate_launch_description():
    """Generate launch description for complete system with Gazebo Harmonic."""
    ...
    # 1. Launch Gazebo Harmonic with Crazyflie simulation
    gazebo_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_kaskazi_drone, 'launch', 'crazyflie_gazebo_harmonic.launch.py')
        ),
        launch_arguments={
    ...
    motion_coordinator_node = TimerAction(
        period=6.0,  # Start after mock MAVROS
        actions=[
            Node(
                package='kaskazi_drone',
    ...
    drone_control_node = TimerAction(
        period=7.0,  # Start after motion coordinator
        actions=[
            Node(
                package='kaskazi_drone',
    ...
    movement_client_node = TimerAction(
        period=9.0,  # Wait for all services to start
        actions=[
            Node(
                package='kaskazi_drone',
    ...
    return LaunchDescription([
        ...
        # System components
        gazebo_sim,              # Gazebo Harmonic + Crazyflie
